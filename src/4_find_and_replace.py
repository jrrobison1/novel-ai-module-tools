...
def get_unique_replacement(original_name, name_type, replacement_list, used_project_pile, used_file_pile):
    for candidate in replacement_list:
        fail = False

        # Replace last names ending with 's' or 'x' with names that also end in 's' or 'x'
        if (name_type.startswith('S') and (original_name.endswith('s') or original_name.endswith('x')) and not (candidate.endswith('s') or candidate.endswith('x'))):
            logger.info(f"Unable to use candidate [{candidate}] for original name [{original_name}] because the replacement does not end with 's' or 'x'")
            continue
        if (name_type.startswith('S') and (candidate.endswith('s') or candidate.endswith('x')) and not (original_name.endswith('s') or original_name.endswith('x'))):
            logger.info(f"Unable to use candidate [{candidate}] for original name [{original_name}] because the original name does not end with 's' or 'x'")
            continue
        ...
        for used_name in original_character_names:
            ...
            if (similarity > ORIGINAL_NAME_SIMILARITY_THRESHOLD):
                logger.info(f"Unable to replace [{original_name}] with [{candidate}] because it is too similar to [{used_name}] which was already an ORIGINAL character name for the project. Similarity is [{similarity}]")
                fail = True
                break
        if fail == True:
        ...
        for used_name in used_project_pile:
            ...
            if (similarity > USED_NAME_IN_PROJECT_SIMILARITY_THRESHOLD):
                logger.info(f"Unable to replace [{original_name}] with [{candidate}] because it is too similar to [{used_name}] which is already in the PROJECT list. Similarity is [{similarity}]")
                fail = True
                break
        if fail == True:
            replacement_list.remove(candidate)
        ...
        for used_name in used_file_pile:
            ...
            if (similarity > USED_NAME_IN_FILE_SIMILARITY_THRESHOLD):
                logger.info(f"Unable to replace [{original_name}] with [{candidate}] because it is too similar to [{used_name}] which is already in the FILE list. Similarity is [{similarity}]")
                fail = True
                break

        replacement_list.remove(candidate)
        if fail == False:
            logger.debug(f"Candidate not found in existing list. Returning candidate [{candidate}]")
            return candidate
    
    logger.debug(f"Returning EMPTY from get_unique_replacement. Original name [{original_name}] Replacement list size [{len(replacement_list)}]")
    return ""
...
def get_replacement(original_name, name_type, used_project_pile, used_file_pile):
    for category, replacement_list in names.items():
        if name_type == category:
            return get_unique_replacement(original_name, name_type, replacement_list, used_project_pile, used_file_pile)
...
def get_ner_file_text(file_name, strip_prefixes):
    ...
    with open(os.path.join(ner_directory, NER_FILE_PREFIX + base_file_name), "r") as ner_file:
...
for file_name in file_names:
    ner_file_text = get_ner_file_text(file_name, ["nosplits_", SPLITS_FIRST_HALF_PREFIX, SPLITS_SECOND_HALF_PREFIX])
...
for file_name in file_names:
    ...
    for ner_line in ner_lines:
        ...
        if (name_type == ""):
            logger.error(f"{file_name} ERROR Unable to make replacement for string: [{original_name}] because no name type was given for this line")
            continue
        else:
            replacement = get_replacement(original_name, name_type, used_project_pile, used_file_pile)

            if replacement == "":
                logger.error(f"{file_name} ERROR Unable to make replacement for string: [{original_name}] because the replacement is EMPTY")
                continue
            used_project_pile.append(replacement)
            used_file_pile.append(replacement)
            input_text = re.sub(r"(^|[ ?!,.();'\"\-–—])%s([ ?!,.();'\"\-–—])" % re.escape(original_name), r"\1%s\2" % re.escape(str.strip(replacement)), input_text, flags=re.MULTILINE)
            logger.info(f"{filename}: Successfully replaced string [{original_name}] with string [{replacement}]")

    replaced_file = open(os.path.join(output_directory, REPLACEMENTS_FILE_PREFIX + file_name), "w")
...
for file_name in file_names:
    base_name = file_name.removeprefix(SPLITS_FIRST_HALF_PREFIX).removeprefix(SPLITS_SECOND_HALF_PREFIX)

    if base_name.startswith("nosplits_"):
        # Open first half
        with open(os.path.join(output_directory, REPLACEMENTS_FILE_PREFIX + base_name), 'r') as input_file:
            full_text = input_file.read()
            input_file.close()
        # Open stitched file for output
        with open(os.path.join(stitched_directory, STITCHED_FILE_PREFIX + base_name.removeprefix("nosplits_")), 'w') as stitched_write_file:
            stitched_write_file.write(full_text)
            stitched_write_file.close()
    else:
        # Open first half
        with open(os.path.join(output_directory, REPLACEMENTS_FILE_PREFIX + SPLITS_FIRST_HALF_PREFIX + base_name), 'r') as input_file:
            first_half = input_file.read()
            input_file.close()
        
        # Open second half
        with open(os.path.join(output_directory, REPLACEMENTS_FILE_PREFIX + SPLITS_SECOND_HALF_PREFIX + base_name), 'r') as input_file:
        ...
        # Open stitched file for output
        with open(os.path.join(stitched_directory, STITCHED_FILE_PREFIX + base_name), 'w') as stitched_write_file:
