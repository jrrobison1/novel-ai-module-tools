...
DEFAULT_PRIMARY_PATTERN = "\w"
DEFAULT_SECONDARY_PATTERN = "\w"
...
try:
    ...
    logger.warning(f"No {CONFIG_FILE_NAME} config file found. "
                   f"Will use only default values")

try:
    SPLITS_FIRST_HALF_PREFIX = config["splits"]["first_half_prefix"]
except:
    logger.warning(f"No config value found for SPLITS_FIRST_HALF_PREFIX. "
...
try:
    ...
    logger.warning(f"No config value found for SPLITS_SECOND_HALF_PREFIX. "
...
try:
    ...
    logger.warning(f"No config value found for NER_FILE_PREFIX. "
...
try:
    ...
    logger.warning(f"No config value found for REPLACEMENTS_FILE_PREFIX. "
...
try:
    ...
    logger.warning(f"No config value found for STITCHED_FILE_PREFIX. "
...
try: 
    ...
    logger.warning(f"No config value found for PRIMARY_PREFIX. "
...
try:
    PRIMARY_SCORE_FIRST_THRESHOLD = config["patterns"]["match_primary_score_first_threshold"]
except:
    logger.warning(f"No config value found for PRIMARY_SCORE_FIRST_THRESHOLD. "
...
try:
    SECONDARY_SCORE_FIRST_THRESHOLD = config["patterns"]["match_secondary_score_first_threshold"]
except:
    logger.warning(f"No config value found for SECONDARY_SCORE_FIRST_THRESHOLD. "
...
try:
    PRIMARY_SCORE_SECOND_THRESHOLD = config["patterns"]["match_primary_score_second_threshold"]
except:
    logger.warning(f"No config value found for PRIMARY_SCORE_SECOND_THRESHOLD. "
...
try:
    SECONDARY_SCORE_SECOND_THRESHOLD = config["patterns"]["match_secondary_score_second_threshold"]
except:
    logger.warning(f"No config value found for SECONDARY_SCORE_SECOND_THRESHOLD. "
...
try:
    ...
    logger.warning(f"No config value found for MATCH_WORD_COUND_THRESHOLD. "
                   f"Using default value of [{DEFAULT_MATCH_WORD_COUND_THRESHOLD}]")
    MATCH_WORD_COUND_THRESHOLD = DEFAULT_MATCH_WORD_COUND_THRESHOLD

try:
    ORIGINAL_NAME_SIMILARITY_THRESHOLD = config["patterns"]["original_name_similarity_threshold"]
except:
    logger.warning(f"No config value found for ORIGINAL_NAME_SIMILARITY_THRESHOLD. "
...
try:
    USED_NAME_IN_PROJECT_SIMILARITY_THRESHOLD = config["patterns"]["used_name_in_project_similarity_threshold"]
except:
    logger.warning(f"No config value found for USED_NAME_IN_PROJECT_SIMILARITY_THRESHOLD. "
                   f"Using default value of [{DEFAULT_USED_NAME_IN_PROJECT_SIMILARITY_THRESHOLD}]")
    USED_NAME_IN_PROJECT_SIMILARITY_THRESHOLD = DEFAULT_USED_NAME_IN_PROJECT_SIMILARITY_THRESHOLD

try:
    USED_NAME_IN_FILE_SIMILARITY_THRESHOLD = config["patterns"]["used_name_in_file_similarity_threshold"]
except:
    logger.warning(f"No config value found for USED_NAME_IN_FILE_SIMILARITY_THRESHOLD. "
                   f"Using default value of [{DEFAULT_USED_NAME_IN_FILE_SIMILARITY_THRESHOLD}]")
    USED_NAME_IN_FILE_SIMILARITY_THRESHOLD = DEFAULT_USED_NAME_IN_FILE_SIMILARITY_THRESHOLD

try:
    LOG_LEVEL = config["logger"]["level"]
except:
    logger.warning(f"No config value found for LOG_LEVEL. "
